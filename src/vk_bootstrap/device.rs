    ...
    while ! next.is_null() {
        match unsafe {(*next).s_type} {
            vk::StructureType::PHYSICAL_DEVICE_VULKAN_1_2_FEATURES => {
                ...
                if features12.buffer_device_address.is_zero() || features12.descriptor_indexing.is_zero() {return false;}
            ...
            vk::StructureType::PHYSICAL_DEVICE_VULKAN_1_3_FEATURES => {
                ...
                if features13.dynamic_rendering.is_zero() || features13.synchronization2.is_zero() {return false;}
...
fn surface_supported(instance : &Instance, surface_loader : &Surface, surface : vk::SurfaceKHR, physical_device : vk::PhysicalDevice) -> Result<u32> {
    unsafe {Ok(instance.get_physical_device_queue_family_properties(physical_device).iter().enumerate()
        .find(|(index, qfp) : &(usize, &QueueFamilyProperties)| -> bool {
            ...
            && surface_loader.get_physical_device_surface_support(physical_device, *index as u32, surface).unwrap()
        }).unwrap().0 as u32)}
}